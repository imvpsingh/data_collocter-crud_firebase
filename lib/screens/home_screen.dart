...
class _HomeScreenState extends State<HomeScreen> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: appBarTitle("DATA", "COLLECTOR",22,22),
        centerTitle: true,
      ),
      body: StreamBuilder<List<Map<String, dynamic>>>(
        stream: DatabaseMethods().fetchUserData(),
                          ...
                          children: [
                            ...
                            InkWell(
                                onTap: (){
                                  ...
                                  locationController.text = userData[index]["location"];
                                  editEmployeeDetails(userData[index]["id"]);

                                },
                                child: const Icon(Icons.edit,size: 18,color: Color(0xff375e97),)),
                            const SizedBox(width: 8,),
                            InkWell(
                                onTap: () async{
                                  await DatabaseMethods().deleteEmployeeDetails(userData[index]['id']);
                                },
                                child: const Icon(Icons.delete_forever,size: 20,color:   Color(0xfffb6542),))
                          ],
                        ),
                        CustomText(
    ...
    content: SizedBox(
      ...
      child: SingleChildScrollView(
        child: Column(
          ...
          child: TextButton(
            child: const CustomText(text: "Update",fontSize: 18,color: Colors.white,shadow: [],),
          onPressed: () async {
            ...
            await DatabaseMethods().updateEmployeeDetails(id,userInfoMap).then((onValue){Navigator.pop(context);});
          }))),
          ],
        ),
      ),
